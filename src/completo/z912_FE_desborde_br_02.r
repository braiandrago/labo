# FE con variables generadas por Braian Drago_02_Interaccion entre variables Importantes
...
AgregarVariables  <- function( dataset )
{
  ...
  #ALGUNAS VARIABLES NUEVAS - RELACIONES CON EDAD (BRAIAN)
  #Se crean relaciones entre movimientos de el cliente y la edad y tambien su antiguedad
  #Se crean relaciones entre movimientos del cliente y acreditaciones
  ...
  dataset[ , visa_mconsumsobrelimite := Visa_mconsumospesos/ Visa_mlimitecompra]
  
  #CLASIFICACION DE LOS CLIENTES (BRAIAN)## 
  #1 al 3 donde 1 son los que tienen antiguedad menor a 12 meses (BRAIAN)##
  ...
  #ACREDITACIONES / MONTOS DE ACREDITACIONES DE HABERES y LIMITE DE COMPRA (BRAIAN)##
  dataset[ , acred_m := rowSums( cbind( mpayroll,  mpayroll2) , na.rm=TRUE ) ]
  dataset[ , limite_de_ingresos_ratio:= ifelse(acred_m == 0, NA, mv_mlimitecompra / acred_m) ]
  
  #PRESTAMOS (BRAIAN)
  dataset[ , cprestamos := rowSums( cbind( cprestamos_personales,  cprestamos_prendarios, cprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , mprestamos_tot := rowSums( cbind( mprestamos_personales,  mprestamos_prendarios, mprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , promed_prestamos:= ifelse(cprestamos == 0, NA, mprestamos_tot / cprestamos) ]
  
  #INVERSIONES - Se decide incluir al PF como inversion (BRAIAN)
  dataset[ , cant_invers := rowSums( cbind( cinversion1, cinversion2, cplazo_fijo) , na.rm=TRUE ) ]
  ...
  dataset[ , tenencia_consolidada := ifelse(tenencia == 0, NA, tot_invers / tenencia) ] 
  
  #COMISIONES - (BRAIAN)
  #Se plantea relacion entre liquidez y comisiones y entre acreditaciones y comisiones
  dataset[ , comisiones_tot := rowSums( cbind( mcomisiones_mantenimiento,  mcomisiones_otras) , na.rm=TRUE ) ]
  dataset[ , liquid_comis := ifelse(liquid_tot == 0, NA,comisiones_tot/ liquid_tot) ] 
  dataset[ , acred_comis := ifelse(acred_m == 0, NA, comisiones_tot/ acred_m ) ] 
  
  #DIGITALIZACION: cantidad de transacciones desde app/ total transacc voluntarias (BRAIAN)
  # y cantidad de transacciones desde app/ transacc de HBank. 
  #se divide por 3 ctrx_quarter para tener el promedio menual ya que contiene los ultimos 90 dias
  dataset[ , digitalizacion := cmobile_app_trx/ (ctrx_quarter/3) ]
  dataset[ , digitalizacion := cmobile_app_trx/ (chomebanking_trx) ]
  
  #RENTABILIDAD (Braian)
  dataset[ , porc_rentab := mrentabilidad/ mrentabilidad_annual ]
  
  #DIVISIONES ENTRE VARIABLES IMPORTANTES (MEJORA CONTINUA!!) (BRAIAN)
  dataset[ , var_div01_liqu_ctr_quart := liquid_tot/ctrx_quarter ]
  dataset[ , var_div02_acredcomis_ctr := acred_comis/liquid_tot ]
  dataset[ , var_div03_visacons_liquid := mtarjeta_visa_consumo/liquid_tot ]
  dataset[ , var_div04_mcuentasald_ctrnorm := mcuentas_saldo/ctrx_quarter_normalizado ]
